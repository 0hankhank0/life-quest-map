...
interface SocialPostCardProps { quote?: AdventureQuote; customQuote?: CustomSocialQuote; mission?: SocialMission; format: SocialCardFormat; showSource?: boolean; className?: string; }
...
export function SocialPostCard({ quote, customQuote, mission, format, showSource = true, className }: SocialPostCardProps) {
  if (!quote && !customQuote && !mission) throw new Error("SocialPostCard requires content.");
  const attribution = quote ? formatAdventureQuoteAttribution(quote) : customQuote ? formatCustomSocialQuoteAttribution(customQuote) : null;
  const quoteText = quote?.text ?? customQuote?.text ?? "";
  const style = { "--social-card-height": `${socialPostHeight(format)}px` } as CSSProperties;
  return <article data-testid="social-post-card" className={`social-post-card social-post-card--${formatClass(format)} ${mission ? "social-post-card--mission" : ""} ${className ?? ""}`} style={style} aria-label={mission ? "Life Quest Map 今日任務貼文" : "Life Quest Map 社群貼文"}>
