...
export function HomePanel({ onNavigate }: { onNavigate: (tab: AppTab) => void }) {
  ...
  const choose = (nextMood: Mood, nextTime: AvailableTime) => { const nextRecommendations = getAdventureRecommendations(microAdventures, { ...recommendationContext, mood: nextMood, time: nextTime }); setMood(nextMood); setTime(nextTime); setDisplayedAdventureId(getNextDisplayedAdventureId(nextRecommendations, adventure.id)); setFormOpen(false); };
  ...
  return <main className="mx-auto max-w-4xl space-y-6 pb-8">
    <header className="px-1 pt-3"><p className="text-sm font-semibold tracking-wide text-emerald-200">Life Quest · {state.profile.name}</p><h1 className="mt-2 text-4xl font-black tracking-[-.045em] text-zinc-50 sm:text-5xl">今天要推進哪一段旅程？</h1><p className="mt-3 max-w-xl text-sm leading-6 text-zinc-300">完成任務、留下片段，讓每次小小的前進都成為你的冒險紀錄。</p></header>

    <section className="game-card overflow-hidden p-4 sm:p-5" aria-label="今日進度">
    ...
    <section className="game-card p-4 sm:p-5" aria-labelledby="week-heading"><div className="flex items-center justify-between"><div><h2 id="week-heading" className="text-xl font-black text-zinc-50">本週旅程</h2><p className="mt-1 text-sm text-zinc-400">每天完成一點，讓連續紀錄延續下去。</p></div><span className="text-sm font-bold text-emerald-100">{week.reduce((total, day) => total + day.completedCount, 0)} 項</span></div><div className="mt-5 grid grid-cols-7 gap-2">{week.map((day, index) => <div key={day.date} className="min-w-0 text-center"><p className="text-xs font-bold text-zinc-400">{weekLabels[index]}</p><div className="mt-2 flex h-20 items-end justify-center rounded-lg bg-white/[0.035] p-1"><div className="w-full rounded-md bg-emerald-300/85 transition-[height]" style={{ height: `${day.completedCount ? Math.max(18, (day.completedCount / weekMax) * 100) : 6}%` }} aria-label={`${formatLocalShortDate(day.date)} ${day.completedCount} 項`} /></div><p className="mt-2 text-xs font-bold text-emerald-100">{day.completedCount}</p></div>)}</div></section>

    <section className="game-card p-4 sm:p-5" aria-labelledby="recent-heading"><div className="flex items-center justify-between"><h2 id="recent-heading" className="text-xl font-black text-zinc-50">最近完成</h2><button type="button" onClick={() => onNavigate("profile")} className="text-sm font-bold text-emerald-200 hover:text-emerald-100">查看歷史</button></div>{recentCompleted.length ? <div className="mt-4 grid gap-2 md:grid-cols-3">{recentCompleted.map((quest) => <article key={quest.id} className="rounded-lg bg-white/[0.04] p-3"><p className="text-xs font-bold text-emerald-100">+{quest.expReward} EXP</p><h3 className="mt-1 line-clamp-2 text-sm font-bold text-zinc-100">{quest.title}</h3><p className="mt-2 text-xs text-zinc-400">{quest.completedAt ? new Date(quest.completedAt).toLocaleDateString() : ""}</p></article>)}</div> : <p className="mt-4 rounded-lg bg-white/[0.04] p-4 text-sm text-zinc-400">完成第一個任務後，這裡會記錄你的最新進展。</p>}</section>

    <section className="game-card p-4 sm:p-5"><h2 className="text-lg font-bold text-zinc-50">此刻想做什麼？</h2><div className="mt-3 flex flex-wrap gap-2">{moods.map((item) => <ChoiceButton key={item.value} active={mood === item.value} onClick={() => choose(item.value, time)}>{item.label}</ChoiceButton>)}</div><div className="mt-5 flex items-center gap-2 text-sm font-bold text-zinc-50"><Clock className="size-5 text-emerald-200" weight="duotone" />可用時間</div><div className="mt-3 flex flex-wrap gap-2">{times.map((item) => <ChoiceButton key={item} active={time === item} onClick={() => choose(mood, item)}>{item === "60" ? "1 小時" : `${item} 分鐘`}</ChoiceButton>)}</div></section>
    <section className="relative overflow-hidden rounded-2xl border border-emerald-200/25 bg-[linear-gradient(135deg,rgba(52,211,153,.18),rgba(24,24,27,.9)_55%)] p-5 shadow-[0_20px_60px_rgba(3,20,15,.3)]"><Sparkle className="absolute right-4 top-4 size-9 text-emerald-200/40" weight="fill" /><p className="text-sm font-semibold text-emerald-100">推薦的微冒險</p><h2 className="mt-2 max-w-md text-2xl font-black tracking-[-.02em] text-zinc-50">{adventure.title}</h2><p className="mt-2 max-w-lg text-sm leading-6 text-zinc-200">{adventure.description}</p><RecommendationReasons reasons={displayedRecommendation.reasons} /><div className="mt-5 flex flex-wrap gap-3"><button type="button" onClick={() => setFormOpen(true)} disabled={completed} className="rounded-xl bg-emerald-300 px-4 py-3 text-sm font-bold text-zinc-950 transition hover:bg-emerald-200 disabled:cursor-not-allowed disabled:opacity-60">{completed ? "今天已完成" : "完成冒險"}</button><button type="button" onClick={() => { setDisplayedAdventureId(getNextDisplayedAdventureId(recommendations, adventure.id)); setFormOpen(false); }} className="inline-flex items-center gap-2 rounded-xl border border-emerald-200/50 px-4 py-3 text-sm font-bold text-emerald-100 transition hover:bg-emerald-300/10"><Shuffle className="size-4" />換一個</button></div><div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => toggleFavoriteAdventure(adventure.id)} className="inline-flex items-center gap-1.5 rounded-lg px-2.5 py-2 text-xs font-semibold text-zinc-200 hover:bg-white/10"><Heart className="size-4 text-emerald-200" weight={state.favoriteAdventureIds.includes(adventure.id) ? "fill" : "regular"} />收藏</button><button type="button" onClick={() => toggleSavedAdventure(adventure.id)} className="inline-flex items-center gap-1.5 rounded-lg px-2.5 py-2 text-xs font-semibold text-zinc-200 hover:bg-white/10"><BookmarkSimple className="size-4 text-emerald-200" weight={state.savedAdventureIds.includes(adventure.id) ? "fill" : "regular"} />稍後再做</button><button type="button" onClick={() => { dismissAdventure(adventure.id); setDisplayedAdventureId(getNextDisplayedAdventureId(recommendations, adventure.id)); }} className="inline-flex items-center gap-1.5 rounded-lg px-2.5 py-2 text-xs font-semibold text-zinc-300 hover:bg-white/10"><Prohibit className="size-4" />不適合現在</button></div></section>
    <section className="game-card flex items-center gap-3 p-4"><CheckCircle className="size-5 text-emerald-200" weight="fill" /><p className="text-sm text-zinc-300">下一個任務，可能就是今天最有成就感的一步。</p><button type="button" onClick={() => onNavigate("quests")} className="ml-auto shrink-0 text-sm font-bold text-emerald-200 hover:text-emerald-100">查看任務</button></section>
