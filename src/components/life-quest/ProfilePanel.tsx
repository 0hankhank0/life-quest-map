      ...
      <section className="game-card p-5">
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <button
            type="button"
            onClick={() => {
              if (!window.confirm(`恢復 Demo 會重建任務、能力值、成就與地圖進度。${user ? "這項變更會同步到目前帳號，其他裝置下次同步後也會受到影響。" : ""}`)) {
                return;
              }
              restoreDemoData();
          ...
          <button
            type="button"
            onClick={() => {
              if (!window.confirm(`重置角色會清空目前資料並回到初始設定。${user ? "這會重置目前帳號的雲端進度，其他裝置下次同步後也會受到影響。" : ""}`)) {
                return;
              }
              resetAppData();
            }}
