...
describe("activity statistics", () => {
  it("uses local dates at the Taiwan midnight boundary", () => {
    ...
    expect(getTodayActivity(quests, new Date("2026-07-15T12:00:00+08:00"), taipei)).toMatchObject({ date: "2026-07-15", completedCount: 1, expEarned: quests[0].expReward });
  });
  ...
  it("returns a Monday-first seven-day summary", () => {
    ...
    const week = getWeekActivity(quests, new Date("2026-07-15T12:00:00+08:00"), taipei);
  ...
  it("builds heatmap data, categories, recent records, and empty summaries", () => {
    ...
    expect(getRecentDayActivity(quests, 3, new Date("2026-07-15T12:00:00+08:00"), taipei).map((day) => day.completedCount)).toEqual([0, 1, 1]);
    ...
    expect(getTodayActivity([], new Date("2026-07-15T12:00:00+08:00"), taipei)).toMatchObject({ completedCount: 0, expEarned: 0 });
  });
});
