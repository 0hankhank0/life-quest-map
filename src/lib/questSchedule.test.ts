...
describe("quest scheduling", () => {
  it("uses local due dates for today, overdue, upcoming, and no due date", () => {
    expect(getDueStatus({ ...base, dueDate: "2026-07-15" }, now)).toBe("today");
    expect(getDueStatus({ ...base, dueDate: "2026-07-14" }, now)).toBe("overdue");
    expect(getDueStatus({ ...base, dueDate: "2026-07-22" }, now)).toBe("upcoming");
  ...
  it("groups tasks for today, upcoming, main, completed, and all", () => {
    ...
    expect(getQuestGuildQuests(quests, "today", now).map((quest) => quest.id)).toEqual(["today", "daily"]);
    expect(getQuestGuildQuests(quests, "upcoming", now).map((quest) => quest.id)).toEqual(["upcoming"]);
